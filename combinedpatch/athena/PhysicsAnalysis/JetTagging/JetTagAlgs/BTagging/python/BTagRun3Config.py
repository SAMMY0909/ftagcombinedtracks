...
def BTagCfg(inputFlags, JetCollection = [], **kwargs):
    ...
    for jet in JetCollection:
        ...
        if timestamp:
            ...
            extraCont.append("xAOD::TrackParticleContainer#InDetCombinedTrackParticles")
            extraCont.append("xAOD::TrackParticleAuxContainer#InDetCombinedTrackParticlesAux.")
            if splitAlg:
...
def JetBTaggerSplitAlgsCfg(inputFlags, JetCollection="", TaggerList=[], SecVertexingAndAssociators = {}, SetupScheme="", **kwargs):
    ...
    for assoc in TrackToJetAssociators:
        result.merge(JetParticleAssociationAlgCfg(inputFlags, jet, "InDetCombinedTrackParticles", assoc, **kwargs))
    ...
    #result.merge(JetSecVertexingAlgCfg(inputFlags, jet, "InDetCombinedTrackParticles", 'MSV', 'BTagTrackToJetAssociatorBB'))
...
def RunHighLevelTaggersCfg(inputFlags, JetCollection, Associator, TrainingMaps, TimeStamp):
    ...
    for dl2 in TrainingMaps:
        result.merge(HighLevelBTagAlgCfg(inputFlags, BTagCollection, 'InDetCombinedTrackParticles', dl2), sequenceName=sequenceName )
